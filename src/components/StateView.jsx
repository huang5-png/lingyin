 ...
 * @param {string} [props.iconType] - 预置图标类型: empty|loading|error|playlist|download|folder|music|search|clock|chart|subtitle|queue|settings|warn|info
 ...
 * @param {boolean} [props.compact] - 紧凑模式（等价于 size="sm"）
...
const StateView = memo(function StateView({
  ...
  const classes = [
    'state-view',
    `state-view-${type}`,
    size && `state-view-size-${size}`,
    compact && 'state-view-compact',
    inline && 'state-view-inline',
    className,
  ].filter(Boolean).join(' ')
