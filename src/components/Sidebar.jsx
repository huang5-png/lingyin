...
export default function Sidebar({ works, selectedWorkId, onSelectWork, onAddFolder, onAddMediaLibrary, cvFilter, circleFilter, onFilterChange, allCVs, allCircles, onOpenSettings, onDeleteWork, viewMode, onViewModeChange }) {
  ...
  const filteredWorks = useMemo(() => {
    ...
    return works.filter((work) => {
      ...
      return title.includes(query) || rjCode.includes(query) || cvs.includes(query) || circle.includes(query) || folderName.includes(query)
    })
  }, [works, searchQuery])
  ...
  return (
    <div className="sidebar">
      ...
      <div className={`work-list ${viewMode === 'grid' ? 'grid-view' : 'list-view'}`}>
                ...
                <>
                  <div className="card-cover">
                    ...
                    <div className="card-overlay">
                      {work.rating > 0 && <span className="card-rating">⭐ {work.rating.toFixed(1)}</span>}
                    </div>
                  </div>
                ...
                <>
                  ...
                  <div className="work-info">
                    ...
                    <div className="work-meta">
                      {work.cvs && work.cvs.length > 0 && <span className="work-cv">{work.cvs[0]}</span>}
                      {work.rating > 0 && <span className="work-rating">⭐ {work.rating}</span>}
                    </div>
                  </div>
                  <button
