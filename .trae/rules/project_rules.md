...
#### 布局结构
- 顶部栏：绝对定位，渐变遮罩，鼠标移动显示/自动隐藏（3秒闲置）
  - 左侧：关闭按钮
  - 中间：作品标题 + 社团名
  - 右侧：翻译切换按钮
...
- 底部信息区：绝对定位浮层，带渐变遮罩（`linear-gradient(to top, rgba(8,8,12,0.95) → transparent)`）
  ...
  - 播放控制栏：进度条、时间显示、播放控制按钮、辅助功能按钮

#### 播放控制功能
- **进度条**：可拖拽/点击跳转，渐变填充，hover 显示圆形滑块
- **时间显示**：当前时间 / 总时长，等宽数字字体
- **播放控制**：上一曲、播放/暂停、下一曲
- **音量控制**：hover 展开滑块，支持静音切换
- **播放速度**：0.5x / 0.75x / 1x / 1.25x / 1.5x / 2x 六档可调
- **书签**：添加当前位置书签，显示当前音频书签数量徽标
- **睡眠定时器**：快捷切换默认倒计时，显示当前状态文本
- **控制栏自动隐藏**：鼠标静止 3 秒后淡出，移动鼠标重新显示
