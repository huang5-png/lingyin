...
### v1.42.0 — 相关作品推荐功能，智能发现相似音声
**发布日期：2026-07-01**

本次更新新增了相关作品推荐功能，基于标签、CV、社团等多维度相似度计算，智能推荐相似作品，帮助您发现更多喜爱的音声。

**✨ 新功能**
- **智能相关推荐** — 在作品详情页的 Related 标签页中查看相关作品推荐
  - 基于标签、CV、社团三维度相似度综合评分
  - 显示相似度百分比，直观了解相关程度
  - 分为三个推荐等级：相似推荐（同标签+同CV+同社团）、同标签作品、同CV作品、同社团作品
  - 点击即可跳转到相关作品
- **LibraryLayout 集成** — 我的库视图支持相关作品推荐
- **DiscoverLayout 集成** — 发现视图（在线作品）也支持相关作品推荐

**🔧 技术实现**
- `calcSimilarity` 函数实现多维度相似度评分算法
- RightTabBar 新增 RelatedPanel 组件，使用 React.memo 优化
- 评分权重：标签 40%、CV 35%、社团 15%、标题 10%
- 相关作品列表最多显示 20 个，同分类最多显示 5 个

### v1.41.0 — 标签管理系统，多标签筛选与自定义颜色
**发布日期：2026-07-01**

本次更新带来了完整的标签管理系统，让您更高效地组织和筛选音声作品。支持自定义标签颜色、批量操作、多标签 AND/OR 筛选等丰富功能。

**✨ 新功能**
- **标签管理面板** — 全新的标签管理界面（左侧导航栏底部标签图标进入）
  - 标签列表按使用数量排序，一目了然
  - 搜索框快速定位标签
  - 支持多选标签进行批量合并
  - 双击标签名快速重命名
  - 点击色点设置标签颜色
- **自定义标签颜色** — 20 色预设调色板，让标签更具辨识度
  - 标签背景和边框基于颜色自动生成
  - 激活态颜色加深，视觉层次清晰
  - 支持清除颜色恢复默认样式
- **多标签筛选** — 筛选系统全面升级，支持同时选择多个标签
  - AND 模式：同时满足所有选中标签（交集）
  - OR 模式：满足任一选中标签（并集）
  - 筛选结果实时计算
- **作品详情标签操作** — 在 WorkDetail 中直接管理标签
  - 点击「+ 添加」按钮快速添加新标签
  - 输入时智能提示已有标签（带颜色和使用数量）
  - 每个标签旁的 × 按钮一键移除
  - 标签显示自定义颜色

**🏗️ 架构优化**
- **useTags Hook** — 封装完整的标签管理逻辑
  - 标签 CRUD、颜色设置、重命名、合并、删除
  - 作品标签增删、批量操作
  - 统一的错误处理和 Toast 通知
- **useFilters 升级** — 标签筛选从字符串升级为数组
  - 向后兼容，平滑过渡
  - 新增 handleToggleTagFilter 便于 UI 交互
  - 新增 allTags 列表（带使用数量统计）
- **数据库层** — 新增 tagMetadata 数据结构
  - 11 个标签相关 IPC 接口
  - 标签元数据纳入数据备份/恢复
  - 完善的批量操作支持

**🔧 技术细节**
- 标签颜色通过 CSS 变量 `--tag-color` 实现，优雅高效
- 标签管理面板使用 StateView 统一空态/加载态
- 所有新增组件使用 React.memo 优化性能
- 构建产物大小保持稳定

### v1.40.0 — 虚拟滚动系统，大规模音声库流畅体验
**发布日期：2026-07-01**

本次更新带来了完整的虚拟滚动系统，彻底解决大规模音声库的列表渲染性能问题，让您在上千个作品中依然能流畅滚动浏览。

**⚡ 性能优化**
- **虚拟滚动 Hook** — 全新 `useVirtualScroll` hook，支持列表和网格两种布局模式
  - 动态列数计算，自动适配容器宽度
  - ResizeObserver 监听容器尺寸变化，响应式调整
  - requestAnimationFrame 优化滚动性能，丝滑流畅
  - 过扫描 (overscan) 缓冲，减少快速滚动时的白屏
- **滚动位置记忆** — 自动保存和恢复滚动位置
  - 我的库 / 收藏视图独立保存，互不干扰
  - localStorage 持久化，重启应用也能恢复
  - 数据加载后智能延迟恢复，避免位置错乱
- **返回顶部按钮** — 滚动超过 300px 时显示
  - 渐变圆角按钮，暖橙主题风格统一
  - 平滑滚动动画，点击即回顶部
  - hover 反馈与过渡动画
- **组件 Memo 化** — 提取 WorkCard 和 WorkRow 为独立组件
  - 自定义比较函数，精确控制重渲染条件
  - 只有真正变化的卡片才会重新渲染
  - 大幅减少长列表的渲染开销

**🏗️ 架构优化**
- **阈值触发策略** — 超过 50 项时自动启用虚拟滚动
  - 小数目不启用，避免不必要的开销
  - 大数目自动开启，保证流畅体验
  - 阈值可配置，灵活适配不同场景
- **完善的 API 设计** — scrollToIndex 支持多种对齐方式
  - start / center / end 三种对齐模式
  - 精确滚动到指定作品位置
- **项目规则同步** — 更新 project_rules.md 文档
  - 新增性能优化与虚拟滚动章节
  - 记录 API 使用规范和最佳实践
  - 新增关键文件职责说明

**🔧 技术细节**
- 使用 transform: translateY() 定位，避免重排
- 支持动态高度预留，便于未来扩展
- 清理机制完善，无内存泄漏
- 构建产物大小保持稳定
...
本次更新新增了完整的音频频谱可视化系统，基于 Web Audio API 实现实时频谱分析，让您在聆听音声的同时，也能欣赏到声音的律动之美。

**✨ 新功能**
- **音频频谱可视化组件** — 基于 Web Audio API AnalyserNode 的实时频谱分析
  - 三种可视化模式：柱状图（bars）、波形图（wave）、圆形频谱（circle）
  - 可配置灵敏度（0.5x - 3x），适配不同音量和曲风
  - Canvas 2D 绘制，支持高 DPI 自动适配，画面清晰锐利
  - requestAnimationFrame 驱动动画，流畅不卡顿
  - 渐变色填充，暖橙主题风格统一
- **播放器频谱集成** — 在波形区域底部叠加 40px 高的频谱条
  - 渐隐遮罩过渡，与波形自然融合
  - 不影响现有波形交互功能
- **沉浸式模式频谱** — 在沉浸式视图底部添加 120px 高的频谱背景
  - 半透明渐变色，增强沉浸氛围
  - 与控制栏布局协调，不遮挡操作
- **可视化设置面板** — 在「设置 → 外观 → 音频可视化」中配置
  - 频谱显示开关
  - 可视化模式切换
  - 灵敏度滑块调节

**🎨 视觉设计**
- 暖橙渐变配色（#c96442 → #ec4899），与整体主题一致
- 圆角柱状图，柔和不刺眼
- 平滑动画过渡，视觉体验舒适
...
- **音频频谱可视化** — 基于 Web Audio API 的实时频谱动画
  - 三种可视化模式：柱状图、波形图、圆形频谱
  - 灵敏度可调（0.5x - 3x），适配不同音量和曲风
  - 集成于播放器波形区和沉浸式模式底部
  - 在「设置 → 外观 → 音频可视化」中配置
