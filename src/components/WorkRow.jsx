...
const WorkRow = memo(function WorkRow({
  ...
  return (
    <div
      className={`work-item row ${isActive ? 'active' : ''} ${isSelected ? 'selected' : ''}`}
      onClick={() => {
        if (bulkMode) {
      ...
      <div className="work-info">
        ...
        <div className="work-meta">
          ...
          {progress && progress.percentage > 0 && (
            <span className="work-progress-pct">{progress.percentage}%</span>
          )}
        </div>
        ...
        {progress && progress.percentage > 0 && (
          <div className="work-progress-bar-container">
            <div className="work-progress-bar" style={{ width: `${progress.percentage}%` }} />
          </div>
        )}
      </div>
      <button
        className={`work-favorite-btn list-favorite ${isFavorited ? 'favorited' : ''}`}
        onClick={(e) => {
          e.stopPropagation()
  ...
  return (
    ...
    prev.work.circle === next.work.circle &&
